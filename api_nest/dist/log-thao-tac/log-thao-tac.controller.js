"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
var LogThaoTacController_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.LogThaoTacController = void 0;
const core_exception_1 = require("../core/exceptions/core.exception");
const helper_service_1 = require("../helper/helper.service");
const main_config_1 = require("../configs/main.config");
const check_permission_decorator_1 = require("../core/decorators/check-permission.decorator");
const user_decorator_1 = require("../core/decorators/user.decorator");
const common_1 = require("@nestjs/common");
const log_thao_tac_dto_1 = require("./dto/log-thao-tac.dto");
const log_thao_tac_service_1 = require("./log-thao-tac.service");
let LogThaoTacController = LogThaoTacController_1 = class LogThaoTacController {
    constructor(logThaoTacService, helperService) {
        this.logThaoTacService = logThaoTacService;
        this.helperService = helperService;
        this.logger = new common_1.Logger(LogThaoTacController_1.name);
    }
    create(createLogThaoTacDto, user) {
        createLogThaoTacDto.user_id = user.id;
        createLogThaoTacDto.user_name = user.ho_va_ten;
        return this.logThaoTacService.create(createLogThaoTacDto);
    }
    async exportExcel(filters, res) {
        filters.limit = -1;
        const data = await this.logThaoTacService.findAllWithPagination(filters);
        if (data.total > 0) {
            const xlsxBuffer = await this.helperService.jsonToXlsx(data.collection);
            return res
                .set('Content-Disposition', `attachment; filename=${Date.now()}.xlsx`)
                .send(xlsxBuffer);
        }
        else {
            throw new core_exception_1.HttpCoreException('Không tồn tại dữ liệu');
        }
    }
    findAll(filters) {
        return this.logThaoTacService.findAllWithPagination(filters);
    }
};
__decorate([
    (0, check_permission_decorator_1.CheckPermission)(main_config_1.ACTION.create),
    (0, common_1.HttpCode)(200),
    (0, common_1.Post)(),
    __param(0, (0, common_1.Body)()),
    __param(1, (0, user_decorator_1.UserReq)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [log_thao_tac_dto_1.CreateLogThaoTacDto, Object]),
    __metadata("design:returntype", void 0)
], LogThaoTacController.prototype, "create", null);
__decorate([
    (0, check_permission_decorator_1.CheckPermission)(main_config_1.ACTION.export),
    (0, common_1.HttpCode)(200),
    (0, common_1.Get)('excel'),
    __param(0, (0, common_1.Query)()),
    __param(1, (0, common_1.Response)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Object]),
    __metadata("design:returntype", Promise)
], LogThaoTacController.prototype, "exportExcel", null);
__decorate([
    (0, check_permission_decorator_1.CheckPermission)(main_config_1.ACTION.index),
    (0, common_1.HttpCode)(200),
    (0, common_1.Get)(),
    __param(0, (0, common_1.Query)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], LogThaoTacController.prototype, "findAll", null);
LogThaoTacController = LogThaoTacController_1 = __decorate([
    (0, common_1.Controller)('log-thao-tac'),
    __metadata("design:paramtypes", [log_thao_tac_service_1.LogThaoTacService,
        helper_service_1.HelperService])
], LogThaoTacController);
exports.LogThaoTacController = LogThaoTacController;
//# sourceMappingURL=log-thao-tac.controller.js.map