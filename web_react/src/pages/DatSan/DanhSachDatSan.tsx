...
import SuaDatSan from './SuaDatSan';
import XemDatSan from './XemDatSan';
import { BaseRow } from '@app/components/common/BaseRow/BaseRow';
import { BaseCol } from '@app/components/common/BaseCol/BaseCol';
import { useTranslation } from 'react-i18next';
import { getDatSanColumns } from './config/columns.config';
import DatSanFilterForm from './components/DatSanFilterForm';

const DanhSachDatSan = ({ path, permission }: { path: string; permission: Actions  }) => {
  ...
  const defaultColumns = getDatSanColumns(
    {
      page: filter.page,
  ...
  return (
    <BaseRow justify='end' gutter={[0, 16]}>
      <BaseCol span={24}>
        <DatSanFilterForm onSearch={handleFormSearch} onReset={handleFormReset} isLoading={isLoading} />
      </BaseCol>
      <BaseCol span={24}>
        <CustomTable
          rowKey='id'
