...
const SuaDatSan = ({ path, id }: { path: string; id: number }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [form] = BaseForm.useForm();
  const dispatch = useAppDispatch();
  const { t } = useTranslation();
  const title = `Sửa ${t('common.dat-san').toLowerCase()}`

  const showModal = async () => {
    const data = await getDataById(id, path);
    form.setFieldsValue({
      ...data,
      ngay_dat: data.ngay_dat ? moment(data.ngay_dat) : undefined,
  ...
  return (
    <>
      ...
      >
        <BaseForm id={`formSuaDatSan-${id}`} form={form} layout='vertical' onFinish={onUpdate}>
          <FormDatSan isEditing/>
        </BaseForm>
      </BaseModal>
    </>
  );
