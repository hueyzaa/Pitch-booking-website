...
import { getDoiTuongColumns } from './config/columns.config';
import DoiTuongFilterForm from './components/DoiTuongFilterForm';

const DanhSachDoiTuong = ({ path, permission }: { path: string; permission: Actions }) => {
  ...
  const defaultColumns = getDoiTuongColumns(
    {
      page: filter.page,
  ...
  return (
    <BaseRow justify='end' gutter={[0, 16]}>
      <BaseCol span={24}>
        <DoiTuongFilterForm onSearch={handleFormSearch} onReset={handleFormReset} isLoading={isLoading} />
      </BaseCol>
      <BaseCol span={24}>
        <CustomTable
          rowKey='id'
...
export default DanhSachDoiTuong;
