...
const SuaDoiTuong = ({ path, id }: { path: string; id: number }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [form] = BaseForm.useForm();
  const dispatch = useAppDispatch();
  const { t } = useTranslation();
  const title = `Sửa ${t('common.doi-tuong').toLowerCase()}`
  ...
  return (
    <>
      ...
      >
        <BaseForm id={`formSuaDoiTuong-${id}`} form={form} layout='vertical' onFinish={onUpdate}>
          <FormDoiTuong isEditing/>
        </BaseForm>
      </BaseModal>
    </>
  );
