...
interface DoiTuongFilterFormProps {
  onSearch: (filters: { field: string; operator: string; value: string }[]) => void;
  onReset: () => void;
  isLoading?: boolean;
}

export const DoiTuongFilterForm = ({ onSearch, onReset, isLoading = false }: DoiTuongFilterFormProps) => {
  ...
  const handleFinish = (values: any) => {
    ...
    if (values.ten_doi_tuong) {
      filters.push({
        field: 'ten_doi_tuong',
        operator: 'contain',
        value: values.ten_doi_tuong.trim()
      });
    }
    ...
    <BaseForm form={form} layout='vertical' onFinish={handleFinish}>
      <BaseRow gutter={[16, 16]}>
        <BaseCol xs={24} sm={12} lg={8}>
          <BaseForm.Item name='ten_doi_tuong' label='Tên đối tượng'>
            <BaseInput placeholder='Nhập tên đối tượng' allowClear size='small' />
          </BaseForm.Item>
        </BaseCol>
...
export default DoiTuongFilterForm;
