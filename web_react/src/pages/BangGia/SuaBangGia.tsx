...
const SuaBangGia = ({ path, id }: { path: string; id: number }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [form] = BaseForm.useForm();
  const dispatch = useAppDispatch();
  const { t } = useTranslation();
  const title = `Sửa ${t('common.bang-gia').toLowerCase()}`
  ...
  return (
    <>
      ...
      >
        <BaseForm id={`formSuaBangGia-${id}`} form={form} layout='vertical' onFinish={onUpdate}>
          <FormBangGia isEditing/>
        </BaseForm>
      </BaseModal>
    </>
  );
