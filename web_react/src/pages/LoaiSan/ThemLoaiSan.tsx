...
import FormLoaiSan from './FormLoaiSan';
import { useTranslation } from 'react-i18next';
import { BaseTypography } from '@app/components/common/BaseTypography/BaseTypography';

const ThemLoaiSan = ({ path }: { path: string }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [form] = BaseForm.useForm();
  const { t } = useTranslation();
  const dispatch = useAppDispatch();
  const title = `Thêm ${t('common.loai-san').toLowerCase()}`;
  ...
  return (
    <>
      ...
      >
        <BaseForm id='formThemLoaiSan' form={form} layout='vertical' onFinish={onCreate}>
          <FormLoaiSan isEditing={false} />
        </BaseForm>
      </BaseModal>
    </>
  );
