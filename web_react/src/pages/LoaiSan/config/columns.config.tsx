...
export const getLoaiSanColumns = (
  ...
  t: any,
  path: string,
  SuaLoaiSan: React.ComponentType<{ path: string; id: number }>
) => {
  return [
    {
      title: 'STT',
    ...
    },
    {
      ...
      render: (id: number, record: any) => {
        return (
          <BaseSpace size={0}>
            {permission.edit && <SuaLoaiSan path={path} id={id} />}
            {permission.delete && (
              <Delete path={path} title={`${t('common.loai-san')} ${record.ten_loai_san}`} id={id} onShow={onRefresh} />
            )}
          </BaseSpace>
        );
      }
    },
    {
      ...
      ...inputSearch({ dataIndex: 'ten_loai_san', operator: 'contain', nameColumn: 'Tên loại sân' })
    },
    {
      title: 'Thời gian tạo',
      dataIndex: 'ngay_tao',
