...
import SuaLoaiSan from './SuaLoaiSan';
import { BaseRow } from '@app/components/common/BaseRow/BaseRow';
import { BaseCol } from '@app/components/common/BaseCol/BaseCol';
import { useTranslation } from 'react-i18next';
import { getLoaiSanColumns } from './config/columns.config';
import LoaiSanFilterForm from './components/LoaiSanFilterForm';

const DanhSachLoaiSan = ({ path, permission }: { path: string; permission: Actions }) => {
  ...
  const defaultColumns = getLoaiSanColumns(
    {
      page: filter.page,
  ...
  return (
    <BaseRow justify='end' gutter={[0, 16]}>
      <BaseCol span={24}>
        <LoaiSanFilterForm onSearch={handleFormSearch} onReset={handleFormReset} isLoading={isLoading} />
      </BaseCol>
      <BaseCol span={24}>
        <CustomTable
          rowKey='id'
...
export default DanhSachLoaiSan;
