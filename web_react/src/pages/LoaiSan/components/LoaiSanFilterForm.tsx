...
interface LoaiSanFilterFormProps {
  onSearch: (filters: { field: string; operator: string; value: string }[]) => void;
  onReset: () => void;
  isLoading?: boolean;
}

export const LoaiSanFilterForm = ({ onSearch, onReset, isLoading = false }: LoaiSanFilterFormProps) => {
  ...
  const handleFinish = (values: any) => {
    ...
    if (values.ten_loai_san) {
      filters.push({
        field: 'ten_loai_san',
        operator: 'contain',
        value: values.ten_loai_san.trim()
      });
    }
    ...
    <BaseForm form={form} layout='vertical' onFinish={handleFinish}>
      <BaseRow gutter={[16, 16]}>
        <BaseCol xs={24} sm={12} lg={6}>
          <BaseForm.Item name='ten_loai_san' label='Tên loại sân'>
            <BaseInput placeholder='Nhập tên loại sân' allowClear size='small' />
          </BaseForm.Item>
        </BaseCol>
...
export default LoaiSanFilterForm;
