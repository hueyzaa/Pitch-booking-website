...
interface SanFilterFormProps {
  onSearch: (filters: { field: string; operator: string; value: string }[]) => void;
  onReset: () => void;
  isLoading?: boolean;
}

export const SanFilterForm = ({ onSearch, onReset, isLoading = false }: SanFilterFormProps) => {
  ...
  const handleFinish = (values: any) => {
    ...
    if (values.ten_san) {
      filters.push({
        field: 'ten_san',
        operator: 'contain',
        value: values.ten_san.trim()
      });
    }

    if (values.id_loai_san) {
      filters.push({
        field: 'san.id_loai_san',
        operator: 'equal',
        value: values.id_loai_san
      });
    }
    ...
    <BaseForm form={form} layout='vertical' onFinish={handleFinish}>
      <BaseRow gutter={[16, 16]}>
        <BaseCol xs={24} sm={12} lg={6}>
          <BaseForm.Item name='ten_san' label='Tên sân'>
            <BaseInput placeholder='Nhập tên sân' allowClear size='small' />
          </BaseForm.Item>
        </BaseCol>
