...
const ThemThuChi = ({ path }: { path: string }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [form] = BaseForm.useForm();
  const { t } = useTranslation();
  const dispatch = useAppDispatch();
  const title = `Thêm ${t('common.thu-chi').toLowerCase()}`;
  ...
  return (
    <>
      ...
      >
        <BaseForm id='formThemThuChi' form={form} layout='vertical' onFinish={onCreate}>
          <FormThuChi isEditing={false}/>
        </BaseForm>
      </BaseModal>
    </>
  );
