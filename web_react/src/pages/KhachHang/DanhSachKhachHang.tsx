...
import XemKhachHang from './XemKhachHang';
import { BaseRow } from '@app/components/common/BaseRow/BaseRow';
import { BaseCol } from '@app/components/common/BaseCol/BaseCol';
import { useTranslation } from 'react-i18next';
import { getKhachHangColumns } from './config/columns.config';
import KhachHangFilterForm from './components/KhachHangFilterForm';

const DanhSachKhachHang = ({ path, permission }: { path: string; permission: Actions }) => {
  ...
  const defaultColumns = getKhachHangColumns(
    {
      page: filter.page,
  ...
  return (
    <BaseRow justify='end' gutter={[0, 16]}>
      <BaseCol span={24}>
        <KhachHangFilterForm onSearch={handleFormSearch} onReset={handleFormReset} isLoading={isLoading} />
      </BaseCol>
      <BaseCol span={24}>
        <CustomTable
          rowKey='id'
