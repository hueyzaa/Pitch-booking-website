...
const SuaDanhGia = ({ path, id }: { path: string; id: number }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [form] = BaseForm.useForm();
  const dispatch = useAppDispatch();
  const { t } = useTranslation();
  const title = `Sửa ${t('common.danh-gia').toLowerCase()}`
  ...
  const onUpdate = async (values: any) => {
    ...
    patchData(path, id, values, closeModel);
    setIsLoading(false);
  };
  ...
  return (
    <>
      ...
      >
        <BaseForm id={`formSuaDanhGia-${id}`} form={form} layout='vertical' onFinish={onUpdate}>
          <FormDanhGia isEditing/>
        </BaseForm>
      </BaseModal>
    </>
  );
