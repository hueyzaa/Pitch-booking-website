...
const ThemDanhGia = ({ path }: { path: string }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [form] = BaseForm.useForm();
  const { t } = useTranslation();
  const dispatch = useAppDispatch();
  const title = `Thêm ${t('common.danh-gia').toLowerCase()}`;
  ...
  return (
    <>
      ...
      >
        <BaseForm id='formThemDanhGia' form={form} layout='vertical' onFinish={onCreate}>
          <FormDanhGia isEditing={false}/>
        </BaseForm>
      </BaseModal>
    </>
  );
